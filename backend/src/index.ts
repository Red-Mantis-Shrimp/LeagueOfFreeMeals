...
app.get( "/api/matches/:id",  ( req, res ) => {
        ...
        request(`${baseURL}lol/match/v4/matches/${req.params.id}?api_key=${apiKey}`,
            (error, response, body) => {
...
app.get( "/api/matchlists/by-account/:account",  ( req, res ) => {
    if(req.params.account in Accounts) {
        ...
        request(`${baseURL}lol/match/v4/matchlists/by-account/${account}?api_key=${apiKey}`,
            (error, response, body) => {
            if (!error && response.statusCode === 200) {
                res.send( body );
            } else {
...
const fillGamesMap = (matches: any, gamesMap: object) =>{
...
app.get( "/api/matches",  ( req, res ) => {
    ...
    Promise.all(requestsArray.map((quest) => {
        return axios.get(quest).then((response) => {
